	...
	for(i = 0; i < 26; i ++)
	{
		if(bitmap & 1 << i)
		{
			...
			switch(i)
			{
				...
				{
					n = sprintf(temp , "%"PRIu32"", ctx->xin->flow.nw_src);
					break;
				}
				case 16: // ovs_be32 nw_dst
				{
					n = sprintf(temp , "%"PRIu32"", ctx->xin->flow.nw_dst);
					break;
				}
...
static void execute_fp_actions(struct xlate_ctx *ctx, struct AT_MATCH_ENTRY *at_entry)
{
	switch(at_entry->act.actype)
	{
		case SAT_OUTPUT:
		{
			...
			ofp_port_t port = (ofp_port_t)at_entry->act.acparam;
	...
	for(i = 0; i < 8; i ++)
	{
        ...
        else
        {
		    ...
		    LIST_FOR_EACH(stt_entry , node, &(stt->stt_entrys))
		    {
			    ...
			    if(param_left == -1 || param_right == -1)
			    {
				    printf("Param type not found\n");
				    return;
			    }
			    ...
			    if(event)
                {
                    ...
                    if(current_state == stt_entry->last_status)
			        {
				        ...
				        printf("Relavant STT entry found\n");
